...
/* C 版 consume (ciface): 收 forward 返回的 logits (unranked memref f32)。
 * 调用链: main -> @refbackend_consume_func_return_mrf32(rank, descriptor) [wrapper, .o 内部]
 *       -> _mlir_ciface_refbackend_consume_func_return_mrf32(sp) [宿主提供, 本函数]
 * wrapper 把 (rank, descriptor) 打包成 {i64, ptr} struct 存栈, 传 struct 指针 sp。
